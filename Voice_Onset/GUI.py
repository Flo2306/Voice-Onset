...
def estimate_remaining_time(iteration, total_iterations, start_time):
    ...
    if iteration > 0:
        estimated_time_per_iteration = elapsed_time / iteration
        remaining_time = remaining_iterations * estimated_time_per_iteration / 60  # Convert to minutes
    else:
        remaining_time = 0
...
time_label = tk.Label(root1, text="Time left: ")
time_label.pack()
...
for iteration in range(len(file_dict)):
    ...
    remaining_time = estimate_remaining_time(iteration, len(file_dict), start_time)

    time_label.config(text="Time left: {}".format(round(remaining_time, 2)) + " minutes left")
